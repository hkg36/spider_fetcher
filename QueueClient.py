...
class QueueClient(object):
    def __init__(self,amqplink,queue_name,needresult=True):
        self.amqplink=amqplink
        ...
        self.Connect()
    def Connect(self):
        self.connection = Connection(self.amqplink)
...
class WeiboQueueClient(QueueClient):
    def __init__(self,amqplink,queue_name,needresult=True):
        QueueClient.__init__(self,amqplink,queue_name,needresult)
        self.APP_KEY=None
...
if __name__ == '__main__':
    import json
    amqplink='amqp://spider:spider@124.207.209.57//spider'

    client=QueueClient('amqp://guest:guest@124.207.209.57//tools','chinese_split')
    ...
    client=TaskQueueClient(amqplink,'net_request',True)
    task=HttpTask()
    ...
    client=HttpQueueClient(amqplink,'net_request',True)
    ...
    client=WeiboQueueClient(amqplink,'weibo_request',True)
